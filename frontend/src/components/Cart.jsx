...
export default function Cart({ isOpen, onClose, onCheckout, onCartUpdate }) {
  ...
  const handleRemoveItem = async (index) => {
    try {
      await fetch(`/api/panier/${index}`, { method: 'DELETE' });
      await fetchRecap();
      onCartUpdate();
    } catch (e) {
      console.error(e);
      ...
      <div className="relative w-full max-w-md bg-slate-50 h-full shadow-2xl flex flex-col animate-fade-in-up sm:animate-none">
        <div className="bg-white px-6 py-5 border-b border-slate-100 flex items-center justify-between">
          <h2 className="text-2xl font-black text-blue-950 flex items-center gap-3">
            ...
            Mon Panier ({recap?.nombreArticles || 0})
          </h2>
          ...
          {!recap || recap.nombreArticles === 0 ? (
            ...
            <div className="space-y-6">
              {recap.articles.map((item, index) => (
                <div key={index} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 relative">
                  <button 
                  ...
                  <div className="flex justify-between items-start mb-2 pr-6">
                    <p className="font-bold text-blue-900">{item.trajet.gare_depart} ➔ {item.trajet.gare_arrivee}</p>
                  </div>
                  <p className="text-sm text-slate-500 mb-4">
                    Départ : {new Date(item.trajet.date).toLocaleDateString()} à {item.trajet.heure_depart}
                    <br/>
                    Type : {item.type}
                  </p>
                  ...
                  <div className="pt-3 border-t border-slate-100 flex justify-between items-center">
                    ...
                    <span className="font-black text-blue-950 text-lg">{item.sousTotal.toFixed(2)}€</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-white p-6 border-t border-slate-100 shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
          <div className="flex justify-between items-end mb-6">
            ...
            <span className="text-3xl font-black text-blue-950">{recap?.prixTotal ? recap.prixTotal.toFixed(2) : "0.00"}€</span>
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <button 
              onClick={onCheckout}
              disabled={!recap || recap.nombreArticles === 0}
